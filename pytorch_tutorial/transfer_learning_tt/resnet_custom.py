...
# Data augmentation and normaolization for training
...
dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=1,  #@UndefinedVariable
                                             shuffle=False, num_workers=4)  # what it the num_workers
              for x in ['train', 'val']}
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['val']:
            ...
            for data in dataloaders[phase]: # phase -> 'train', or 'val'
                ...
                # statistics
                running_loss += loss.data[0] * inputs.size(0) # what is the input size?
...
if __name__ == '__main__': # if runtime error is occur, use this 'if' sentense
    ...
    # Observe that all parameters are being optimized
    # learn only fc layer's parameter
